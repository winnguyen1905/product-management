...
if(buttonOptionPostImage) {
    buttonOptionPostImage.addEventListener("click", (e) => {
        ...
        if(buttonOptionPostImage.innerHTML == "Upload Image File") {
            ...
            inputChange.innerHTML = '<input type="text" placeholder="Your Image Link" input-image-text class="form-control-file" id="avatar" name="avatar" style="padding: 7px; border-radius: 5px">';
            imageText = document.querySelector("[input-image-text]");
        } else {
            buttonOptionPostImage.innerHTML = "Upload Image File";
            inputChange.innerHTML = '<input type="file" input-image-upload class="form-control-file" style="padding: 4px; border-radius: 5px" id="avatar" name="avatar" accept="image/*">';
